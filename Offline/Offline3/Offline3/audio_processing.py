...
max_freq = sample_rate / (2 * interval_step)
num_freqs = len(data_sampled)
frequencies = np.fft.rfftfreq(len(data_sampled), d=1/sample_rate)
...
# Step 2.1: Visualize the frequency spectrum
plt.figure(figsize=(12, 6))
plt.plot(frequencies, ft_magnitude)
...
# Step 3: Filter out unwanted noise frequencies
# Identify the noise frequencies by inspecting the frequency spectrum
noise_freqs = [400, 800, 1200]  # Example: frequencies with large amplitudes

# Create a filtered Fourier Transform
filtered_ft_real = ft_real.copy()
filtered_ft_imag = ft_imag.copy()

for freq in noise_freqs:
    index = np.abs(frequencies - freq).argmin()
    filtered_ft_real[index] = 0
    filtered_ft_imag[index] = 0

# Step 3.1: Visualize the filtered frequency spectrum
filtered_ft_magnitude = np.sqrt(filtered_ft_real**2 + filtered_ft_imag**2)
plt.figure(figsize=(12, 6))
plt.plot(frequencies, filtered_ft_magnitude)
plt.title("Filtered Frequency Spectrum (Unwanted Frequencies Removed)")
plt.xlabel("Frequency (Hz)")
plt.ylabel("Magnitude")
plt.show()
    ...
    for i, t in enumerate(sampled_times):
        for j, freq in enumerate(frequencies):
            ...
            reconstructed_signal[i] += (
                ft_real[j] * cos_component - ft_imag[j] * sin_component
            )
...
# Step 4.1: Reconstruct the signal using IFT
filtered_data = inverse_fourier_transform(filtered_ft_real, filtered_ft_imag, frequencies, sampled_times)
